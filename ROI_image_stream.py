...
class ROI_image_stream():
    def __init__(self,path_dataset, ROI_size):
        ...
        if sorted(self.path_dataset.glob('*.mkv')):
            self.path_video = next(self.path_dataset.glob('*.mkv'))
            print('ROI_image_stream : found *.mkv')
        elif sorted(self.path_dataset.glob('*.avi')):
            self.path_video = next(self.path_dataset.glob('*.avi'))
            print('ROI_image_stream : found *.avi')
        else:
            print('ROI_image_stram : Can not find video file in %s', self.path_dataset)
    ...
    def trainBackgroundSubtractor(self, stride=2000, start_empty=False):
        ...
        start_empty : bool : is animal present from the beginning of the video. True uses high learning rate for initial 1 sec video.
        """
    ...
    def extractROIImage(self, frame_number, dilate_size=2, erosion_size=5, drawPlot=False, prevPoint=None, distance_criterion = 50):
        ...
        def drawDebugPlot():
            ...
            ax[1,0].set_title('Erode + Dilate')
            ax[1,1].imshow(denoised_mask)
            ax[1,1].set_title('Erode + Dilate + Erode')
        ...
        if len(detected_blob) > 1:# if multiple blob is detected, select the largest one
            ...
            for i, blob in enumerate(detected_blob):
                if max_blob_size < blob.size:
                    ...
                    max_blob_index = i
            print('ROI_image_stream : Multiple blobs(%d) detected in frame %d' % (len(detected_blob), frame_number))
            showChosenImage = True
        elif len(detected_blob) == 0 :
            drawDebugPlot()
            raise(BaseException("ROI_image_stream : No blob is detected from %d frame" % frame_number))
        else:
